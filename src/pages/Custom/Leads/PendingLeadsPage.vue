<template>
  <div>
    <BreadCrumb pageTitle="My Pending Leads" />

    <CustomTable :propData="dataToPass" :callBack="moreRecords" :viewDetails="allItemActions" buttonText="View Offer" />
          ...
          <div class="modal-body ps-0 pe-0 pb-0 pt-15 pt-md-25">
            <form @submit.prevent="updateStatus">
              <div class="row">
                ...
                <template v-if="updateForm.status == 'did not answer'">
                  <div class="col-lg-12 col-md-6">
                    <div class="form-group mb-15 mb-md-25">
                      <label class="d-block text-black fw-semibold mb-10">
                        Call Back Time (Optional)
                      </label>
                      <input type="datetime-local" v-model="updateForm.call_back_time"
                        class="form-control shadow-none rounded-0 text-black" placeholder="Select Time" />
                    </div>
                  </div>
                </template>
    ...
    methods: {  
      ...
      moreRecords(url, search){
        if(url != null && typeof url != undefined){
          Lead.pendingLeads(search, url).then(data => {
            this.retrieveRecords(data)
          });
        }

        if(search != null && typeof search != undefined){
          Lead.pendingLeads(search).then(data => {
            this.retrieveRecords(data)
          });
        }
      },
