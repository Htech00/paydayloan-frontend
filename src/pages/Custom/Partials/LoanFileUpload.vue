...
export default defineComponent({
  ...
  methods: {
    ...
    async uploadFile() {
      ...
      try {
          Loan.updateFile(router.currentRoute.value.params.id, formData).then(data => {
                this.uploadingImage = false;
                toast.success('Loan file updated successfully')
                this.audioUrl = "";
                this.selectedFile = null;
            this.callBack(true, 'uploadLoanFileModal');
            }).catch(error => {
                this.uploadingImage = false;
                this.callBack()
                this.audioUrl = "";
