        ...
        <div class="modal-content p-15 p-md-40">
          ...
          <div class="modal-body ps-0 pe-0 pb-0 pt-15 pt-md-25">
            <div class="text-lg text-center mb-3">
              Liquidation Amount : <br/><span class="fs-3">{{ formatNumber(createForm.liquidationAmount ?? 0) }}</span>
            </div>
            ...
            <table class="ps-0 mb-0 w-100">
              ...
              <tbody>
                <tr class="border border-light" v-for="item in selectedLoan?.scheduled_deductions" :key="item.id">
                  ...
                  <td class="p-2 text-left w-25">
                    <div class="badge"
                      :class="{ 'text-outline-success': !item.active, 'text-outline-primary': item.active && !dateHasPassed(item.due_date), 'text-outline-danger': item.active && dateHasPassed(item.due_date) }">
                      {{ item.active && !dateHasPassed(item.due_date) ? 'ACTIVE' : (item.active &&
                      dateHasPassed(item.due_date) ? 'OVERDUE' : 'COMPLETED') }}
                    </div>
                  </td>
                </tr>
              </tbody>
...
import Admin from "@/apis/Admin";
  ...
  export default defineComponent({
    ...
    computed: {
      ...
      downloadReport() {
        return this.hasAnyPermission(['download-portfolio-report'])
      }
    },
    mounted() {
      ...
      Deduction.allDeductions().then(data => {
        this.allLoans = data.data.data.data;
    ...
    methods: {
      ...
      showRepaymentSchedule(itemId) {
        let loan = this.allLoans.filter(e => e.id == itemId);
      ...
      showRepaymentDialog(itemId){
        if(this.hasAnyPermission(['add-repayment'])){
          ...
          this.selectedLoan = loan[0]
          console.log('loan:', loan);
          this.createForm.liquidationAmount = loan[0].liquidation_amount;
      ...
      moreRecords(url, search){
          ...
          if(url != null && typeof url != undefined){
            Deduction.allDeductions(url, search).then(data => {
              this.allLoans = data.data.data.data;
              this.retrieveRecords(data)
            });
          }
      ...
      retrieveRecords (response) {
        ...
        const mappedData = responseData.data.data.map((item) => [
          ...
          item.mandate_status,
         item.mandate_checked_at ? this.formatDate(item.mandate_checked_at) : null,
          item.mandate_reactivation_count,
          item.status,
          item.id
        ]);
